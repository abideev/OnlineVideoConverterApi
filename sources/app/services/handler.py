...
def callback_converter(ch, method, properties, body):
    ...
    if data_dict['convert'] == "false" and data_dict['download'] == "false":
        ...
        return_video_convert("None", data_dict['url'], properties.message_id, data_dict['format'], data_dict['size'],
                             data_dict['bitrate'])
        ch.basic_ack(delivery_tag=method.delivery_tag)
    elif data_dict['convert'] == "true" and data_dict['email'] != "":
        return_video_convert(data_dict['email'], data_dict['url'], properties.message_id, data_dict['format'],
                             data_dict['size'], data_dict['bitrate'])
        ch.basic_ack(delivery_tag=method.delivery_tag)
